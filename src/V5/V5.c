...
void forward(NeuralNetwork* net, double* input, double* hidden, double* output) {
    ...
    #pragma acc data present(net, net->W1, net->W2, net->b1, net->b2, hidden, output, input)
    {
...
void backward(NeuralNetwork* net, double* input, double* hidden, double* output, double* target) {
    ...
    #pragma acc data present(net, net->W1, net->W2, net->b1, net->b2, hidden, output, input) \
        copy(d_output[0:BATCH_SIZE * OUTPUT_SIZE], \
        d_hidden[0:BATCH_SIZE * HIDDEN_SIZE], \
        gradient_W1[0:INPUT_SIZE * HIDDEN_SIZE], \
        gradient_W2[0:HIDDEN_SIZE * OUTPUT_SIZE], \
        gradient_b1[0:HIDDEN_SIZE], \
        gradient_b2[0:OUTPUT_SIZE])
    {
        #pragma acc parallel loop collapse(2)
        for (int b = 0; b < BATCH_SIZE; b++) {
        ...
        #pragma acc update self(output[0:BATCH_SIZE * OUTPUT_SIZE])
    }
}
...
void train(NeuralNetwork* net, double* images, double* labels, int numImages) {
    ...
    for (int epoch = 0; epoch < EPOCHS; epoch++) {
        ...
        #pragma acc data \
            copy(net[0:1], \
                net->W1[0:INPUT_SIZE * HIDDEN_SIZE], \
                net->W2[0:HIDDEN_SIZE * OUTPUT_SIZE], \
                net->b1[0:HIDDEN_SIZE], \
                net->b2[0:OUTPUT_SIZE], \
                hidden[0:BATCH_SIZE * HIDDEN_SIZE], \
                output[0:BATCH_SIZE * OUTPUT_SIZE], \
                images[0:numImages * INPUT_SIZE])
        ...
        for (int i = 0; i < numImages; i += BATCH_SIZE) {
            ...
            #pragma acc update host(output[0:BATCH_SIZE * OUTPUT_SIZE])
